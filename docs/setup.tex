...
We are going to have a user ``recorder'' who will own and run the recorder
application. This user also has to be a member of the audio group.
...
We need to be able to easily connect via ssh to the recorder user.  Set this up with
...
cd .ssh
cp /home/pi/.ssh/* .
chown recorder.recorder *
\end{lstlisting}

Now connect as the ``recorder'' user (\texttt{ssh recorder@recirder}), firstly to check that it works, but also
